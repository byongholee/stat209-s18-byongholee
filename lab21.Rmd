...
**Answer**: It has a similar shape/model of a statistical inference. The distribution begins to rise at the 10 score and peaks at 15 and back down to - at 18. The most count of scores was 15. 
...
**Answer**: The confidence intervals for warmth measures in Hungary are lower than the overall scores of all countries. For the entire world, the estimate is almost 1 score higher than Hungary's. This could be that Hungarian citizens show less warmth to others than Americans or people form other countries. 
...
```{r}
male<- filter(pf, gender == "male")
male

model<-lm_basic(warmth ~ 1, data = male)
reg_table(model, level = .95)
```
```{r}

model<-lm_basic(warmth ~ 1 + gender, data = pf)
reg_table(model, level = .95)
```

**Answer**: The male confidence interval is 13.82 and the female is 14.22. 
...
**Answer**: I don't believe that there is a statistically significant difference between the gender's average scores. Whle the difference does show that females show more warmth than males, I think the numerical difference is quite small. However, this does confirm traditional gender stereotypes that women are more compassionate than men.  
...
```{r}
model<-lm_basic(warmth ~ 1 + fourties, data = pf)
reg_table(model, level = .95)
qplot(warmth,age , data = pf)
```
...
**Answer**:The difference is significantly different between 30s and 40s score, because they do not cross 0. The direction is that people in the 40s are generally warmer than those in their 30s. This could be due to increased age, which equals a protective nature to others. 
...
```{r}
model<-lm_basic(warmth ~ fct_lump(country, 5), data=pf)
reg_table(model, level=.95)
```
...
**Answer**: The US has the highest warmth score while Other and India has the lowest. 
...
**Answer**: Because slope is different than 1, it shows that sensistivity is less likely to affect your overall score, especially for those that 
...
**Answer**: The slope for gender is negative for male. 

Add predictions from the previous model to `pf`.

```{r}
model <- lm_basic(warmth ~ 1 + baseline + gender, data = pf)
warmthpred <- add_prediction(pf, model)
pf
```
...
```{r}
ggplot(warmthpred, aes(baseline, warmth)) +
  geom_point() +
...
**Answer**: The values have a strong positive correlation. It warmth continues to go up when baseline increases, but females have more warmth. 
