...
```{r}
active_birds<- filter(bird_type, n>=5)
active_birds
```
...
```{r}
ggplot(data=bird_type, aes(egg_mass_median, male_mass_median))+
  geom_point(alpha=.2)+
  scale_x_log10()+
  scale_y_log10()+
  annotate("text", x=50, y=25, label="Red points are birds types with 5 or more birds")+
  geom_point(data=active_birds, color="red")
