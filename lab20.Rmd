...
**Answer**: The slope is increasing, it is debatable whether its a strong correlation. I believe it is strong in the beginning, but not towards the end, it suddently cuts off 
...
**Answer**: The intecept confidence interval is between -2282 an -2231. 

What range of values does the confidence interval give for the slope
in this model?

**Answer**: For the slope, the confidence interval is 7756, with the range being 7729 and 7784.
...
**Answer**: Yes, I would be suprised because it is drastically lower than the previous slope interval and does not fall within the confidence interval range.

Would you be surprised if we collected another set of diamonds, in exactly the
same way as before, and found a slope estimate equal to 7500?

**Answer**: No, I would not be suprised because it falls within the confidence intnerval range. 
...
**Answer**: The slope in this linear regression only takes values that are positive, this is important because it shows positive correlation. 
...
reg_table(model, level=.95)
qplot(depth, price, data=diamonds)+geom_smooth(method = "lm")
```

What does the model predict will be the price of a diamond with a depth of
60?

**Answer**: 3,984.67 (taking depth x 60 + intercept (5763.67))

Describe in words the estimate of the slope in terms of the dataset.

**Answer**: The estimate of the slope is a weak negative correlation. 
...
**Answer**: It contains both positive and negative values. 
...
**Answer**: Depth=(-102 x 50)5100, +(7765.1x2)+4045.3. I dont think this is right, but 24,675.

Are both of the slopes in the model statistically significant? What are
their signs?

**Answer**: Both slopes are statistically significant, as they both are within the 95% confidence intervals. 
...
**Answer**: The largest positive residual is 12683.655
...
**Answer**: The slope increases, which means that price increases as it gets lights. The deeper (depth) you are the industrial use diamonds, well cut diamonds are usually more expensive.
...
**Answer**: I expect the one with 1 carats in wieght to be heavier diamond to cost simply due to the slope of less depth equals heavier price. 
