...
ggplot(birds, aes(male_mass, egg_mass))+
  ...
  annotate("text", x = 100, y=1, label = "Parrots are the red points")+
  scale_x_log10()+
  scale_y_log10()
```
...
ggplot(birds, aes(male_mass, egg_mass))+
  geom_point()+
  annotate("text", x = 218.7, y=77, label = "Red-tailed Tropicbird")+
  geom_smooth(method = "lm", color = "black", se = FALSE, linetype = "dashed", size = 0.5)+
  scale_x_log10()+
  scale_y_log10()
```
...
ggplot(birds, aes(male_mass, egg_mass))+
  geom_point(alpha =.2)+
  geom_point(color="blue", data = owls)+
  annotate("text", x = 600, y=4, label = "Owls are the blue points")+
  ggtitle("Owls and Eggs")+
  scale_x_log10()+
  scale_y_log10()
```
